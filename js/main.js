...
function setupEventListeners() {
    ...
    document.querySelector('.sidebar-item.active').addEventListener('click', navigateToHome);
}
...
async function loadVideoById(videoId) {
    try {
        ...
        document.getElementById('video-date').textContent = formatDate(video.uploadDate);
        ...
        const watchHistory = window.watchHistory || { addToWatchHistory: () => {}, incrementVideoViews: () => {} };
...
async function loadRecommendedVideos(currentVideoId) {
    try {
        ...
        const videos = await response.json();
        const recommendedVideos = videos.filter(video => video.id !== parseInt(currentVideoId));
        const recommendedContainer = document.getElementById('recommended-container');
            ...
            videoElement.innerHTML = `
                ...
                <div class="recommended-details">
                    ...
                    <div class="recommended-stats">
                        ${formatNumber(video.views)} views • ${formatTimeAgo(video.uploadDate)}
                    </div>
                </div>
            `;
